...
export default function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const frameInterval = window.innerWidth < 768 ? 1000 / 30 : 1000 / 36;
    let lastRenderAt = 0;
    const renderer = new Renderer({
      dpr: dpr ?? Math.min(window.devicePixelRatio || 1, 1.35),
      alpha: true,
      antialias: true,
    });
    ...
    const loop = (time) => {
      ...
      if (lastRenderAt && time - lastRenderAt < frameInterval) {
        return;
      }
      lastRenderAt = time;
      renderFrame(time);
